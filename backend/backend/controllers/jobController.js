...
const addJob = asyncHandler(async (req, res) => {
  ...
  if (response) {
    ...
    await ReportsClient.findOneAndUpdate(filter, update, {
      upsert: true,
      new: true,
    });
    ...
    return res
      .status(400)
      .json({ errorMessage: "Error While  Adding Job Data" });
  }
});
...
const updateJob = asyncHandler(async (req, res) => {
  ...
  if (result) {
    await saveNotifcation(
      ...
      id,
      result._id,
      "Have Successfully Updated Your Job ",
      result.title
    );
    await result.save();
...
const deletejob = asyncHandler(async (req, res) => {
  ...
  if (jobmodel) {
    ...
    await saveNotifcation(
      ...
      id,
      _id,
      "Have Successfully Deleted Your Job ",
      jobmodel.title
    );
    ...
    if (result.deletedCount == 1) {
      ...
      if (response.deletedCount) {
        return res
          .status(200)
          .json(
            `Jobs And ${response.deletedCount} Submitted Proposals On Jobs  Successfully Deleted`
          );
      } else {
        return res
          .status(200)
          .json(`Jobs And 0 Submitted Proposals On Jobs  Successfully Deleted`);
      }
    } else {
...
const submitProposal = asyncHandler(async (req, res) => {
  ...
  /*  const attachment = bufferConversion(
    req?.file?.originalname,
    req?.file?.buffer
  );

  const { secure_url } = await cloudinary.uploader.upload(attachment);  */
...
//getJobProposalsForFreelancer with job id

const getJobProposalsForFreelancerSingleJob = asyncHandler(async (req, res) => {
  // Check for freelancer
  if (!req.freelancer) {
    return res
      .status(401)
...
/// getJobProposals for freelancer with auth of freelancer
const getJobProposalsForFreelancer = async (req, res) => {
  // Check for freelancer
  if (!req.freelancer) {
    return res
      .status(401)
...
/// getJobProposals for client with auth of client
const getJobProposalsForClient = async (req, res) => {
  // Check for client
  if (!req.client) {
    return res
      .status(401)
  ...
  const clinetExist = await Client.findById(id);
...
/// getJobProposals for client with job id
...
const getMyJobs = asyncHandler(async (req, res) => {
  ...
  let response = await jobModel.find({ client: id });
...
const acceptJobProposal = asyncHandler(async (req, res) => {
  ...
  if (result) {
    await saveNotifcation(
      ...
      result.submittedBy,
      _id,
      "Has Accepted your Job Proposal",
      "Has Accepted your Job Proposal"
    );
    await result.save();
    ...
    const order = await OrderModel.create({
      proposalId: _id,
      freelancer: response.submittedBy,
      price: doc.bid,
      client: id,
      type: "job",
      jobOrderType: doc,
...
const rejectJobProposal = asyncHandler(async (req, res) => {
  ...
  await saveNotifcation(
    ...
    result.submittedBy,
    _id,
    "Has Rejected Your Job Proposal!",
    "Has Rejected Your Job Proposal!"
  );
  await result.save();
  const response = await jobProposal.findOne({ _id });
