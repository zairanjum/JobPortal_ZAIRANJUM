...
import { getAllJobs, DeleteJob } from "../../../../redux/actions/AdminActions";
...
const AllJobs = () => {
  ...
  const allJobs = useSelector((state) => state?.adminData?.jobs);
  const { jobsLoading } = useSelector((state) => state?.adminData);
          ...
          {allJobs && allJobs.length > 0 ? (
            <div className="w-full overflow-hidden rounded-lg shadow-xs">
              ...
              <Pagination
                ...
                totalFreelancers={allJobs?.length}
                paginate={paginate}
