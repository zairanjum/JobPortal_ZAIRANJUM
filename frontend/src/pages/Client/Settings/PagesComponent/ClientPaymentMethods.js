...
const PaymentMethods = () => {
  ...
  const { loading, payments } = useSelector((state) => state?.clientData);
  const userId = useSelector((state) => state?.clientData?.userInfo?._id);

  useEffect(() => {
    dispatch(getPayment(userId, toast));
  }, [dispatch, userId]);
  ...
  const onSubmit = async (formValues) => {
    dispatch(addPayment(formValues, navigate, toast));
  };
  ...
  const delPayment = (paymentId) => {
    dispatch(deletePayment(paymentId, toast));
    setDeleteModal(false);
  };
