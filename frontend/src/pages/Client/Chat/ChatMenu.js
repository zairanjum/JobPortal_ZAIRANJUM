...
import { GetAllMessages } from "../../../redux/actions/ClientActions";
...
const ChatMenu = () => {
  ...
  const { messages, loading } = useSelector((state) => state?.clientData);
  ...
  if (messages && messages?.length > 0) {
    sortedData = messages
      .filter((message) => message?.receiver_client?._id === id)
      .sort((a, b) => {
