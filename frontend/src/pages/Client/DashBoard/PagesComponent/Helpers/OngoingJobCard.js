...
const OngoingJobCard = ({ data }) => {
  const id = data?._id;
  const storedProgress = sessionStorage.getItem(`jobProgressClient-${id}`);
      ...
      interval = setInterval(() => {
        setProgress((prevProgress) => {
          ...
          if (newProgress > 100) {
            ...
            sessionStorage.removeItem(`jobProgressClient-${id}`); // remove the stored progress
            return 100;
          }
          sessionStorage.setItem(`jobProgressClient-${id}`, newProgress); // store the new progress in sessionStorage
          return newProgress;
        });
      }, intervalTime);
    }
