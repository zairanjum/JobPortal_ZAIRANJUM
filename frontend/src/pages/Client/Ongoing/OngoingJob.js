...
const OngoingJob = () => {
  ...
  const [days, setDays] = useState(
    sessionStorage.getItem(`jobDaysClient-${id}`) || duration
  );
  const [hours, setHours] = useState(
    sessionStorage.getItem(`jobHoursClient-${id}`) || 0
  );
  const [minutes, setMinutes] = useState(
    sessionStorage.getItem(`jobMinutesClient-${id}`) || 0
  );
  const [seconds, setSeconds] = useState(
    sessionStorage.getItem(`jobSecondsClient-${id}`) || 0
  );

  useEffect(() => {
    sessionStorage.setItem(`jobDaysClient-${id}`, days);
    sessionStorage.setItem(`jobHoursClient-${id}`, hours);
    sessionStorage.setItem(`jobMinutesClient-${id}`, minutes);
    sessionStorage.setItem(`jobSecondsClient-${id}`, seconds);
