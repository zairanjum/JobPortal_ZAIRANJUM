...
import { UpdateExchangeSkills } from "../../../redux/actions/FreelancerActions";
import { useDispatch, useSelector } from "react-redux";
import Spinner from "../DashBoard/PagesComponent/Helpers/Spinner";

const EditExchangeSkillForm = () => {
  const navigate = useNavigate();
  const dispatch = useDispatch();
  const { loading } = useSelector((state) => state?.freelancerData);
  const exchangeSkillDetail = useLocation().state.myExchangeSkill;
  ...
  const formData = {
    ...
    price: price,
    tags: tags,
    _id: exchangeSkillDetail._id,
  };

  const onSubmit = (formValues) => {
    dispatch(UpdateExchangeSkills(formValues, navigate, toast));
  };
...
export default EditExchangeSkillForm;
