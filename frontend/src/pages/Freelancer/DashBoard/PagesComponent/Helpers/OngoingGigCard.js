...
const OngoingGigCard = ({ data }) => {
  const id = data?._id;
  const storedProgress = sessionStorage.getItem(`gigProgressFreelancer-${id}`);
      ...
      interval = setInterval(() => {
        setProgress((prevProgress) => {
          ...
          if (newProgress > 100) {
            ...
            sessionStorage.removeItem(`gigProgressFreelancer-${id}`); // remove the stored progress
            return 100;
          }
          sessionStorage.setItem(`gigProgressFreelancer-${id}`, newProgress); // store the new progress in sessionStorage
          return newProgress;
        });
      }, intervalTime);
    }
