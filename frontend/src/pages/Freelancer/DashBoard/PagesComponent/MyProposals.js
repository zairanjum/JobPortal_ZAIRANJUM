...
import {
  GetAllJobsProposalsForFreelancer,
  DeleteJobProposal,
...
const Proposals = () => {
  ...
  const proposals = useSelector(
    (state) => state?.freelancerData?.proposals?.response
  );


  useEffect(() => {
    dispatch(GetAllJobsProposalsForFreelancer(toast));
  }, [dispatch]);
  ...
  const acceptedProposals =
    (proposals?.filter &&
      proposals?.filter((jobProposal) => jobProposal?.status === 1)) ||
    [];
  ...
  const pendingProposals =
    (proposals?.filter &&
      proposals?.filter((jobProposal) => jobProposal?.status === 0)) ||
    [];
      ...
      {loading ? (
        ...
        <div className="container  mx-auto grid">
          <div className="flex flex-row flex-wrap  items-center justify-between mb-4">
            <Title title="Pending Submitted Proposals" />
          </div>
                          ...
                          <tr key={index} className="text-gray-700 ">
                            <td className="px-4 py-3">
                              <div className="flex items-center text-sm">
                                <Link
                                  to="/freelancer/proposal-details"
                                ...
                                >
                                  <div>
                                    <p className="font-semibold">
                                      {" "}
                                      {limitText(proposal?.job?.title, 5)}..
                                    </p>
                                  </div>
                                </Link>
                              </div>
