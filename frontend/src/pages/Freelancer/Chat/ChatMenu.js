...
import { GetAllMessages } from "../../../redux/actions/FreelancerActions";
...
const ChatMenu = () => {
  ...
  const { messages, loading } = useSelector((state) => state?.freelancerData);
  ...
  if (messages && messages?.length > 0) {
    sortedData = messages
      .filter((message) => message?.receiver_freelancer?._id === id)
      .sort((a, b) => {
