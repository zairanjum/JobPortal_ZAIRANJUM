...
const OngoingJob = () => {
  ...
  const otherId = selectUser(
    id,
    singleJob?.freelancer?._id,
    singleJob?.client?._id
  );
  ...
  const [days, setDays] = useState(
    sessionStorage.getItem(`jobDaysFreelancer-${singleJobId}`) || duration
  );
  const [hours, setHours] = useState(
    sessionStorage.getItem(`jobHoursFreelancer-${singleJobId}`) || 0
  );
  const [minutes, setMinutes] = useState(
    sessionStorage.getItem(`jobMinutesFreelancer-${singleJobId}`) || 0
  );
  const [seconds, setSeconds] = useState(
    sessionStorage.getItem(`jobSecondsFreelancer-${singleJobId}`) || 0
  );

  useEffect(() => {
    sessionStorage.setItem(`jobDaysFreelancer-${singleJobId}`, days);
    sessionStorage.setItem(`jobHoursFreelancer-${singleJobId}`, hours);
    sessionStorage.setItem(`jobMinutesFreelancer-${singleJobId}`, minutes);
    sessionStorage.setItem(`jobSecondsFreelancer-${singleJobId}`, seconds);
              ...
              <div className="flex flex-col flex-wrap w-auto flex-grow px-4 my-4 space-y-3">
                <div className="item">
                  <div className="flex px-2 py-2 text-sm font-thin text-gray-700 bg-gray-300 rounded-md ">
                    ...
                    <span className="-mt-1">
                      Duration: {singleJob?.jobOrderType?.job?.duration} days
                    </span>
                  </div>
                </div>
                <div className="item">
                ...
                <div className="item">
                  <div className="flex px-2 py-2  text-sm font-thin text-gray-600 bg-gray-300  rounded-md ">
                    ...
                    <span className="-mt-1">
                      Est.Budget: ${singleJob?.jobOrderType?.job?.budget}
                    </span>
                  </div>
                </div>
                <div className="item">
