...
import { GetGuestFreelancers } from "../../../redux/actions/GuestActions";
import { GetReviews } from "../../../redux/actions/GuestActions";
import { useSelector, useDispatch } from "react-redux";
import EmptyState from "../Helpers/EmptyState";
import Spinner from "../Helpers/Spinner";

const FreelancerFilterDesktop = () => {
  const dispatch = useDispatch();
  const freelancers = useSelector((state) => state.guestData.guestfreelancers);
  const reviews = useSelector((state) => state?.guestData?.guestReviews);
  const { loading } = useSelector((state) => state.guestData);

 

  useEffect(() => {
    dispatch(GetGuestFreelancers(toast));
    dispatch(GetReviews(toast));
  }, [dispatch]);

  useEffect(() => {
    setFilterFreelancer(freelancers);
  }, [freelancers]);
  ...
  const handleFilterData = useCallback(
    (selectedFilters) => {
      setFilterFreelancer(selectedFilters);
    },
    [setFilterFreelancer]
  );
          ...
          <>
            ...
            <section className="pt-6 pb-6">
              <div className="grid grid-cols-1 gap-x-8 gap-y-8 lg:grid-cols-4">
                ...
                {filterFreelancer && filterFreelancer.length > 0 ? (
                      ...
                      <div className="mx-4 mt-4">
                        {hasFreelancers(
                          ...
                          hasFreelancers(
                            ...
                            <Link
                              ...
                              state={{ singleFreelancer: freelancer, reviews }}
                            >
                              <SingleFreelancer
                                data={freelancer}
                                reviews={reviews}
                        ...
                        <div>
                          <Pagination
                            ...
                            totalFreelancers={filterFreelancer?.length}
                            paginate={paginate}
...
export default FreelancerFilterDesktop;
